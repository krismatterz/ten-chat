...
type Conversation = {
  ...
  branchFromMessageId?: string; // Now string instead of Id<"messages">
};
...
export function AppSidebar() {
  ...
  const conversations = useQuery(api.conversations.list, { limit: 100 });
  ...
  const groupedConversations = useMemo(() => {
    ...
    const pinned = filteredConversations.filter((conv: any) => conv.isPinned);
    const todayConvs = filteredConversations.filter(
      (conv: any) => !conv.isPinned && new Date(conv.updatedAt) >= today
    );
